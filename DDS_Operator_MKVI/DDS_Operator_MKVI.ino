...
Software_Reset()
{
  ...
  void myTest() {
    ...
    Ramp r1 = Ramp(5000000, 50000000, 10000, 0, 0); Ramp r2 = Ramp(2000000, 5000000, 8000, 0, 0); Ramp r3 = Ramp(1000000, 2000000, 8000, 0, 0);
    Ramp myArray[] = {r1, r2, r3}; profile0.produceFreq(10 * 1000 * 1000);
    Serial.println(F("Beginning Chained Sweep Test")); Serial.println(F("Begin")); //sweeper.chainedSweeps(myArray, 3); Serial.println(F("Finished"));
  }
  ...
  void setup() {
    //Start the serial port at 9600 baud Serial.begin(9600); Serial.println(F("ARDUINO BOOTED"));
    Serial.print("Amount of free RAM pre-setup: "); Serial.println(freeMemory()); //From LCD_Handler; sets up LCD display
    setupLCD();
    //From SD_Handler; sets up SD card
    setupSD();
    //From Ethernet_Handler; sets up Ethernet Host //Passed DHCP_ENABLE which is True/False setup_Ethernet(DHCP_ENABLE);
  ...
  boolean handleSweeps() {
    boolean funcStatus = false; switch (client.read()) {
      case 'r':
        rampArrayLen = loadRamps( rampArray);
      case 'l':
        funcStatus = sendOutRamps(rampArrayLen, rampArray); break;
      case 'f': break;
        break;
    }
    return funcStatus;
  ...
  boolean handleConstFreq() {
    char c = client.read(); if (c == 'f') {
      memset(smallBuffer, 0, 10);
      client.readBytesUntil(':', smallBuffer, 10);
      int freq = atoi((char*)smallBuffer);
      Serial.print("Producing Single-Mode Frequency: "); Serial.println(freq); profile0.produceFreq(freq);
      return true;
    }
    //
    // }
    ...
    char mainCharHandle(char readValue) {
      int returnVal = 1;
      boolean commandStatus = false; switch (readValue) {
        case 'c':
          commandStatus = handleConstFreq(); break;
        case 's':
          commandStatus = handleSweeps(); break;
        case 'G': sweeper.chainedSweeps(rampArray, rampArrayLen); commandStatus = true;
          break;
        case 'i': commandStatus = handleInformation(); break;
        case '!':
          client.println("Stop function"); break;
        case 'r': returnVal = 'r'; break;
        case 'p':
          commandStatus = sendAddressAcross(); break;
        case
            't'
            :
          myTest();
...
; :
        ...
        loop()
      {
        ...
        print
        (F(
           "Ethernet server established at: "));
        Serial
        .
        println
        (
      ...
      while (!client) {
        ...
        (
          ...
          (
            10);
          //Set timeout on client operations to 10 ms
          ...
          if (!navigateTo
              (
                ...
                , 8 ,
                200)) {
            //nagateTo function from Ethernet_Handler
            ...
            else
            {
              ...
              println
              (F(
                 "SD card was not initialized correctly"));
            }
          }
          else
          {

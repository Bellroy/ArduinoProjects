...
void myTest() {
  ...
  //sweeper.chainedSweeps(myArray, 3); Serial.println(F("Finished"));
}
...
void setup() {
  ...
  Serial.print("Amount of free RAM pre-setup: ");
  Serial.println(freeMemory()); //From LCD_Handler; sets up LCD display
  setupLCD();
  ...
  fullSetup();
  Serial.print("Amount of free RAM post-setup: ");
...
boolean handleConstFreq() {
  ...
  if (c == 'f') {
    ...
    Serial.print("Producing Single-Mode Frequency: ");
    Serial.println(freq);
...
void loop() {
  ...
  Serial.print("Amount of free RAM: ");
  Serial.println(freeMemory());
  ...
  Serial.print(F("Ethernet server established at: "));
      ...
      if (SDisBegun) {
        ...
        if (myFile) {
          ...
          while (myFile.available() && client.connected()) {
            ...
            memset(smallBuffer, 0, 500);

            myFile.read(smallBuffer, sizeof(smallBuffer));


            client.write(smallBuffer, sizeof(smallBuffer));
          }
